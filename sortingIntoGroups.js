...
const AMOUNT_OF_SENTENCES = SENTENCES_TO_SORT["sortToGroups"]["drag"].length; // לשנות בתהאם לכמות המשפטים

const CODE = 1; // הקוד שמקבלים בסוף הלומדה
...
function addElements() {   
    ...
    shuffle(SENTENCES_TO_SORT["sortToGroups"]["drag"]);
    // 
    for (let nCount = 0; nCount < SENTENCES_TO_SORT["sortToGroups"]["drag"].length; nCount++) {
        $(".sentences").append("<div class='drag-sentences' id='sen" + (nCount + 1) +"' >" + SENTENCES_TO_SORT["sortToGroups"]["drag"][nCount].drag + "</div>");
        
        //
        if (SENTENCES_TO_SORT["sortToGroups"]["drag"][nCount].group === 0) {
...
function finished() {
    ...
    $(`.end-title`).html(`כל הכבוד! <br> הקוד הוא ${CODE}`)
    // הוספת כפתור המשך
